...
dřívějších dobách komunikace probíhala pouze přes příkazovou
řádku, dnes tento způsob ustupuje ve prospěch grafického uživatelského
rozhraní (\emph{GUI}). Oba způsoby mají svá pro a proti, například
...
vyhodnocením testování a následným opravám.
 \end{description}
\subsubsection{ArcGIS}
% - temer nekonecna hloubka zanoreni dialogu
% - zalozky dvourade
% - ovladani zoomovani koleckem naopak
% - legenda zacne wizardem, kdyz ji editujeme, je to jiny dialog
% - chybi advanced settings, zejmena pri nastaveni textu
% - chyba ve wizardu u site - zpet se vraci na defaultni nastaveni
%
% + obrazky k nabidkam (napr na prave tlacitko)
% + wizard casty (legenda, sit)
\subsubsection{QGIS}

\subsubsection{gvSIG}
% -nekonzistence v dvojteckach, OK vs Accept, nektere dialogy pouzivaji
group boxy, jine ne
...
% % Aplikace je napsána primárně pro GRASS 7 a měla by fungovat i v
% GRASS 6.5. %a testována pod operačním systémem Linux, distribuce Ubuntu
% 10.04 LTS.
