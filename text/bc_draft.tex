...
\begin{abstract}
Cílem této  bakalářské práce  je vytvoření grafického uživatelského
...
GUI umožňuje interaktivní tvorbu mapových výstupů a
tím ji uživa\-telům usnadňuje. GUI implementuje větší část funkcionality modulu
ps.map a je napsáno pomocí grafické knihovny wxPython.

Text bakalářské práce se dále věnuje problematickým částem  ps.map a
zároveň obsahuje i  porovnání vybraných programů pro tvorbu mapových výstupů
z~hlediska jejich funkcionality a GUI.
\bigskip
...
\begin{abstract}
The aim of this bachelor thesis was to develop a
graphical user interface
(GUI) for composing hardcopy map outputs in the GRASS GIS. Hardcopy maps
GUI allows to compose  maps interactively to make it more
comfortable for users. GUI implements most of the ps.map functionality and it is
...
creating hardcopy map outputs in terms of their functionality and graphical
interface.
\bigskip
...
systémy v~kate\-gorii svobodného softwaru. Je to mimo jiné výborný nástroj
...
komfortnějšího ovládání tohoto modu\-lu přes GUI.
...
bylo třeba proz\-koumat do nejmenších podrobností.
